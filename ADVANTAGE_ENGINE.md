...
**World Monitor ingest gate (fixture protocol):** a synthetic observation batch can be held `pending` in a dedicated `world_pending_observations` table (exact bytes + SHA-256) with no allow path, so untrusted ingest evidence never silently becomes an authoritative observation, component, or promotion while it is fixture-sourced. See `WORLD_MONITOR_ADAPTER.md`; operator outreach remains a separate boundary in `EXPERIMENT_OUTREACH_PREP.md`.
